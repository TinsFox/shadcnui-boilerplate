...
export const getShortcutKeys = (keys: string[]): ShortcutKeyResult[] => keys.map((element) => getShortcutKey(element))
...
export const isUrl = (
  ...
  const { requireHostname = false } = options
  if (text.includes("\n")) return false
...
const base64MimeType = (encoded: string): string => {
  ...
  return result.length > 1 ? result[1]! : "unknown"
}
